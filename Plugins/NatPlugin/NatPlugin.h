...
#ifndef __NatPlugin_H_1F469DAF__
#define __NatPlugin_H_1F469DAF__
...
/**
    ...
    on how to design a processor that inherits from NatPlugin.

    @see ProcessorGraph, GenericEditor, SourceNode, FilterNode, LfpDisplayNode
*/
class PLUGIN_API NatPlugin   : public AudioProcessor
                                    , public PluginClass
...
public:
    ...
    */
    ...
    virtual void setPathToProcessor (NatPlugin* p);

    /** Sets a processor's source node.*/
    virtual void setSourceNode (NatPlugin* sn);

    /** Sets a processor's destination node.*/
    virtual void setDestNode (NatPlugin* dn);

    /** Sets one of two possible source nodes for a merger.*/
    virtual void setMergerSourceNode (NatPlugin* sn);

    /** Sets one of two possible source nodes for a splitter.*/
    virtual void setSplitterDestNode (NatPlugin* dn);
    ...
    virtual bool canSendSignalTo (NatPlugin*) const;
	...
	@see NatPlugin::getProcessorFullId(uint16,uint16) */
...
private:
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (NatPlugin);
};


#endif  // __NatPlugin_H_1F469DAF__
